...
<template>
  <div class="grid gap-8">
    <UiCard :title="doc.title" :description="doc.description">
      ...
      <p class="text-sm leading-7 text-slate-500">{{ doc.summary }}</p>
    </UiCard>

    <section class="grid gap-4">
      <div class="space-y-1">
        <h2 class="text-2xl font-semibold tracking-tight text-slate-950">在线 Demo</h2>
        <p class="text-sm leading-6 text-slate-500">演示真实交互效果，并与代码示例保持同步。</p>
      </div>
      <slot />
    </section>
    ...
    <section class="grid gap-4">
      <div class="space-y-1">
        <h2 class="text-2xl font-semibold tracking-tight text-slate-950">导入方式</h2>
    ...
    <section v-if="doc.props?.length" class="grid gap-4">
      <div class="space-y-1">
        <h2 class="text-2xl font-semibold tracking-tight text-slate-950">Props</h2>
        <p class="text-sm leading-6 text-slate-500">
    ...
    <section v-if="doc.events?.length" class="grid gap-4">
      <div class="space-y-1">
        <h2 class="text-2xl font-semibold tracking-tight text-slate-950">Events</h2>
        <p class="text-sm leading-6 text-slate-500">
    ...
    <section v-if="doc.slots?.length" class="grid gap-4">
      <div class="space-y-1">
        <h2 class="text-2xl font-semibold tracking-tight text-slate-950">Slots</h2>
        <p class="text-sm leading-6 text-slate-500">
    ...
    <section v-for="section in doc.sections ?? []" :key="section.title" class="grid gap-4">
      <div class="space-y-1">
        <h2 class="text-2xl font-semibold tracking-tight text-slate-950">{{ section.title }}</h2>
      </div>
    ...
    <section v-if="doc.examples?.length" class="grid gap-4">
      <div class="space-y-1">
        <h2 class="text-2xl font-semibold tracking-tight text-slate-950">使用示例</h2>
        <p class="text-sm leading-6 text-slate-500">
